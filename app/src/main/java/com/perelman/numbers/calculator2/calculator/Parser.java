...
import static com.perelman.numbers.calculator2.calculator.Element.ElementType.BRACKET;
import static com.perelman.numbers.calculator2.calculator.Element.ElementType.CONSTANT;
import static com.perelman.numbers.calculator2.calculator.Element.ElementType.FUNCTION;
import static com.perelman.numbers.calculator2.calculator.Element.ElementType.NUMBER;
import static com.perelman.numbers.calculator2.calculator.Element.ElementType.SIGN;
import static com.perelman.numbers.calculator2.calculator.Element.ElementType.VAR;
...
class Parser<T> {
    ...
    private Element.ElementType type(char c) {
        ...
        if (helper.isType(c)) {
            ...
            type = Element.ElementType.FUNCTION;
        }
        return type;
    }
    ...
    private void normaliseLetters(List<Element> list) {
        for (int i = list.size() - 1; i >= 0; --i) {
            ...
            if (e.type == Element.ElementType.FUNCTION) {
                    ...
                    for (int j = e.symbol.length() - 1; j >= 0; --j) {
                        ...
                        if (helper.hasName(s) || helper.isVar(s)) {
                            Element nE = new Element(s,
                                    (helper.funcs.getFunc(s) != null) ? Element.ElementType.FUNCTION :
    ...
    private void findUnarySigns(List<Element> list){
        ...
        if(last.type == Element.ElementType.SIGN && helper.signs.getSign(last.symbol).canBeUnary){
            last.type = Element.ElementType.FUNCTION;
        }
        for(int i = 1; i < list.size(); ++i){
            ...
            if(e.type == Element.ElementType.SIGN && helper.signs.getSign(e.symbol).canBeUnary &&
                ...
                e.type = Element.ElementType.FUNCTION;
            }
            last = e;
        }
    }
